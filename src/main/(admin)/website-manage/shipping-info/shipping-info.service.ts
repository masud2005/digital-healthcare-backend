...
export class ShippingInfoService {
    ...
    async get() {
                ...
                data: {
                    ...
                    policies: {
                        create: [
                            ...
                            {
                                text: "P.O. boxes may not be eligible for cold-chain meds",
                                order: 6,
                            },
        ...
        if (partnerSection && partnerSection.partners) {
            partnerSection.partners = await Promise.all(
                partnerSection.partners.map(async (partner) => {
                    if (partner.logo?.fileUrl) {
                        partner.logo.fileUrl = await this.storageService.getSignedUrl(
                            partner.logo.fileUrl,
                        );
                    }
                    return partner;
                }),
    ...
    async update(dto: UpdateShippingInfoDto) {
        ...
        if (dto.shippingTimelineSection) {
            ...
            if (dto.shippingTimelineSection.steps) {
                await this.prisma.shippingTimelineStep.deleteMany({
        ...
        if (dto.shippingPolicySection) {
            ...
            if (!section) {
                section = await this.prisma.shippingPolicySection.create({
                    data: {
                        ...
                        disclaimerDescription:
                            dto.shippingPolicySection.disclaimerDescription || "",
                    },
                });
            } else {
            ...
            if (dto.shippingPolicySection.policies) {
                await this.prisma.shippingPolicyItem.deleteMany({
                    where: { sectionId: section.id },
                });
