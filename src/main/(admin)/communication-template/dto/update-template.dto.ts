...
export class UpdateTemplateDto {
    @ApiPropertyOptional({
        description: "Email subject line. Supports Handlebars variables e.g. {{subject}}",
    ...
    @ApiPropertyOptional({
        description:
            "Override the email header title for this specific template (overrides global layout default)",
        example: "Secure login verification",
    })
    @IsString()
    @IsOptional()
    ...
    @ApiPropertyOptional({
        description:
            "Override the email header subtitle for this specific template (overrides global layout default)",
...
Bold Text: Wrap text in **double asterisks** for bold, e.g. **10 minutes**`,
    ...
    @ApiPropertyOptional({
        description:
            "Whether to show the info cards (Secure Access & Need Help) at the bottom of the email",
        example: true,
    })
    @IsBoolean()
    @IsOptional()
    ...
    @ApiPropertyOptional({
        description: "Body text of the first info card",
        example:
    ...
    @ApiPropertyOptional({
        description: "Title of the second info card (help section)",
        example: "NEED HELP?",
    })
    @IsString()
    @IsOptional()
    ...
    @ApiPropertyOptional({
        description: "Body text of the second info card",
        example:
